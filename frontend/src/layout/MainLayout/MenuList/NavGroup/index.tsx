...
export default function NavGroup({ item, lastItem, remItems, lastItemId, setSelectedID }: NavGroupProps) {
    ...
    const checkOpenForParent = (child: NavItemType[], id: string) => {
        child.forEach((ele) => {
            ...
            if (ele?.url && !!matchPath({ path: (ele as any)?.link ? (ele as any).link : ele.url, end: true }, pathname)) {
                setSelectedID(id);
            }
        });
    };
    ...
    const checkSelectedOnload = (data: NavItemType) => {
        ...
        childrens.forEach((itemCheck) => {
            ...
            if (itemCheck?.url && !!matchPath({ path: (itemCheck as any)?.link ? (itemCheck as any).link : itemCheck.url, end: true }, pathname)) {
                setSelectedID(currentItem.id);
            }
        });

        if (data?.url && !!matchPath({ path: (data as any)?.link ? (data as any).link : data.url, end: true }, pathname)) {
            setSelectedID(currentItem.id);
        }
    };
