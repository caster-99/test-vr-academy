...
export default function NavItem({ item, level, isParents = false, setSelectedID }: NavItemProps) {
    ...
    const isSelected = !!matchPath({ path: (item as any)?.link ? (item as any).link : item.url!, end: false }, pathname);
        ...
        <>
            ...
            >
                ...
                {(drawerOpen || (!drawerOpen && level !== 1)) && (
                    <Tooltip title={item.title as string} disableHoverListener={!hoverStatus}>
                        <ListItemText
                            primary={
                                <Typography
                                    ref={ref}
